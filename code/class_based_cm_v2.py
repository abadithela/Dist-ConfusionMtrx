# Class-labeled confusion matrix for the mini dataset
import os
import pdb
from utils import *
...
class GenClassCM():
    def __init__(self, dataroot=None, traindir="trainval-all", ntrain_dirs =10, save_data_ext=False, chk_stored_scenes=False, horizon=100, discrete_horizon=10, class_dict={0: 'pedestrian', 1:'obstacle', 2:'empty'}, sensors=['CAM_FRONT', 'CAM_FRONT_LEFT', 'CAM_FRONT_RIGHT'], cm_type="class_based", cm_fname=None):
    ...
    def get_dirname(self):
        if self.save_data_ext:
            dirname = "/Volumes/Extreme SSD/cm_processing/" + self.traindir + "/matchings_class"
        else:
            cwd = os.getcwd()
    ...
    def construct_confusion_matrix(self, scene_obj):
        ...
        for i in range(len(discretization)):
            ...
            if objs_at_dist_i == []: # If no object at that distance return empty
                true_class = 'empty'
                pred_class = 'empty'
                row = self.rev_class_dict[pred_class]
                col = self.rev_class_dict[true_class]
                scene_cm[i][row, col] += 1
            else:
                obj_det = True
                for true_pred_tup in objs_at_dist_i:
                    pred_class = true_pred_tup[1]
                    true_class = true_pred_tup[0]
                    scene_cm[i] = self.process_anns_at_j(pred_class, true_class, scene_cm[i])
        return scene_cm

    def process_anns_at_j(self, prediction, gt_ann, scene_cmi):
        row = self.rev_class_dict[prediction]
        col = self.rev_class_dict[gt_ann]
...
if __name__ == '__main__':
    ...
    if example =="full":
        cm_type = "class_based"
        cm_fname = "full_3class_cm_ped_obs_cam_f_hz_"
    ...
    if example =="full1":
        cm_type = "class_based"
        cm_fname = "full1_3class_cm_ped_obs_cam_f_hz_"
